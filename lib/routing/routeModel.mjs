...
export const ROUTE_ERROR_STATUS = {
  ...
  [ROUTE_ERROR_CODES.METHOD_NOT_ALLOWED]: 405,
...
export function normalizeRouteStep(step = {}) {
  const type =
    typeof step.type === "string" || typeof step.type === "number"
      ? String(step.type)
      : typeof step.maneuverType === "string" || typeof step.maneuverType === "number"
        ? String(step.maneuverType)
        : null;
  const streetNames = Array.isArray(step.streetNames)
    ? step.streetNames.filter((streetName) => typeof streetName === "string" && streetName)
    : typeof step.streetName === "string" && step.streetName
      ? [step.streetName]
      : [];

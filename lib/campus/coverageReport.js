...
const { createIndoorCoverageReport } = require("./indoor/coverage.js");
...
function createAccessibilityCoverage(buildings, entrances) {
  const entrancesWithRampMetadata = entrances.filter((entrance) => entrance.ramp === true || entrance.ramp === false);
  const entrancesWithStairMetadata = entrances.filter((entrance) => entrance.stairs === true || entrance.stairs === false);
  ...
  return {
    ...
    ramps: {
      known: entrancesWithRampMetadata.length,
      present: entrances.filter((entrance) => entrance.ramp === true).length,
      absent: entrances.filter((entrance) => entrance.ramp === false).length
    },
    stairs: {
      known: entrancesWithStairMetadata.length,
      present: entrances.filter((entrance) => entrance.stairs === true).length,
      absent: entrances.filter((entrance) => entrance.stairs === false).length
    },
    elevators: {
      imported: 0,
      status: "not-imported"
    ...
    pedestrianAccessibilityMetadata: {
      imported: 0,
      status: "not-imported"
    },
...
function createCampusCoverageReport() {
  ...
  const verifiedBuildings = buildings.filter((building) => isOfficialSource(building.source));
  const provisionalBuildings = buildings.filter((building) => isProvisionalSource(building.source));
  const verifiedEntrances = entrances.filter((entrance) => isOfficialSource(entrance.source));
  const provisionalEntrances = entrances.filter((entrance) => isProvisionalSource(entrance.source));
  const buildingsWithVerifiedEntrances = new Set(verifiedEntrances.map((entrance) => entrance.buildingId));
  ...
  return {
    ...
    buildings: {
      discovered: buildings.length,
      imported: buildings.length,
      verified: verifiedBuildings.length,
      provisional: provisionalBuildings.length,
      missingCoordinates: buildings.filter((building) => !building.centroid).map((building) => building.id)
    },
    entrances: {
      total: entrances.length,
      verified: verifiedEntrances.length,
      accessible: entrances.filter((entrance) => entrance.accessible === true).length,
      provisional: provisionalEntrances.length,
      buildingsWithZeroVerifiedEntrances: buildings
        .filter((building) => !buildingsWithVerifiedEntrances.has(building.id))
        .map((building) => building.id)
    },
    sourceStatus: {
    ...
    indoor: createIndoorCoverageReport(),
    ...
    paths: {
      ...
      importedPedestrianSegments: 0,
      accessibilityMetadataSegments: 0,
      routingReadiness: "valhalla-osm-ready-official-campus-paths-not-imported",
    ...
    readiness: {
      status: verifiedEntrances.length > 0 ? "partial-official-data" : "provisional-data-needs-official-gis",
      officialBuildingCoveragePercent: buildings.length
        ? Math.round((verifiedBuildings.length / buildings.length) * 100)
        : 0,
      officialEntranceCoveragePercent: entrances.length
        ? Math.round((verifiedEntrances.length / entrances.length) * 100)
        : 0,
      routableBuildingCoveragePercent: buildings.length
        ? Math.round((buildings.filter((building) => entranceBuildingIds.has(building.id) || building.centroid).length / buildings.length) * 100)
        : 0,
      highestImpactMissingDatasets: [
        "official entrance points with accessibility metadata",
        "official pedestrian path network with stairs, ramps, and closures",
        "official building polygons and identifiers",
        "official bike-rack inventory",
        "official residential building inventory"
      ]
    }
  };
}
